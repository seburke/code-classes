 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a few numbers") {
    ...
    test("sum empty list") (assert(sum(List()) === 0))

    test("sum negatives") (assert(sum(List(1,2,-4)) === -1))

    test("sum repeats") (assert(sum(List(2,2,2)) === 6))
  ...
  test("max of a few numbers") {
    ...
    test("max repeated max") (assert(max(List(1,2,3,3,3)) === 3))

    test("max negatives") (assert(max(List(-10, -3)) === -3))
